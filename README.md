...
For more details, see, e.g., the discussion in Gelman and Carlin's [*Bayesian Data Analysis*](http://www.stat.columbia.edu/~gelman/book/BDA3.pdf) (3rd Ed, 2014), regarding "The full Bayesian treatment of the hierarchical model" (Chapter 5).
...
5. Edit the data module, `module_<projectname>_data.py`:
    ...
    * Within the `prep_data()` method, manage the import of the raw data file into a dataframe of standard form, and define multiple `data_analysis_subproject`(s):
        ...
        - Sort the dataframe such that: the names of the individuals have a known order (the order will be used in the model hypothesis module for setting initial guess values in the `indiv_pop` and `indiv_nopop` dataframes), and the `'X'` values for each data set are ordered.
